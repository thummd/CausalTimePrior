"""Generate dataset with SHUFFLED causal graph relationships as a control.

This creates training data where query_types are randomly assigned rather than
determined by the true causal graph structure. If a model trained on this shuffled
data performs similarly to the original model, it would suggest the model is NOT
learning causal structure but instead exploiting distributional artifacts.
"""
...
def generate_dataset_shuffled(
    n_scms: int,
    T: int,
    max_nodes: int,
    seed: int = 42,
    output_path: str = "data/causal_time_prior_shuffled_10k.pt",
):
    """Generate dataset with SHUFFLED query type assignments.
    
    Query types are randomly assigned (uniform distribution) rather than
    determined by causal graph structure. This breaks the causal relationship.
    """
    print("=" * 80)
    print(f"Generating SHUFFLED Dataset (Control): {n_scms} SCMs, T={T}")
    print("=" * 80)
    ...
    query_targets_list = []
    query_times_list = []
    query_types_list = []  # SHUFFLED: random assignment
    ...
    print(f"\n2. Generating {n_scms} SCM pairs with SHUFFLED query types...")
    for i in range(n_scms):
        ...
        # SHUFFLED: Sample query variable randomly (not based on causal graph)
        query_target = np.random.randint(0, N)
        
        # SHUFFLED: Assign query type randomly (uniform distribution)
        query_type = np.random.choice(3, p=[0.33, 0.33, 0.34])  # Balanced
    ...
    dataset = {
        ...
        'query_types': query_types_tensor,  # SHUFFLED (random assignment)
        ...
        'metadata': {
            ...
            'max_nodes': max_nodes,
            'seed': seed,
            'shuffled': True,  # Flag to indicate this is a shuffled control
        }
    }
    ...
    print(f"   SHUFFLED: Query types randomly assigned (no causal relationship)")
    print(f"\n   Query type distribution (should be ~uniform):")
    print(f"     - Type 0: {query_type_counts[0]}/{n_scms} ({100 * query_type_counts[0] / n_scms:.1f}%)")
    print(f"     - Type 1: {query_type_counts[1]}/{n_scms} ({100 * query_type_counts[1] / n_scms:.1f}%)")
    print(f"     - Type 2: {query_type_counts[2]}/{n_scms} ({100 * query_type_counts[2] / n_scms:.1f}%)")
    print(f"\n   Saved to: {output_path}")
    
    print("\n" + "=" * 80)
    print("Shuffled dataset generation complete!")
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate shuffled dataset")
    ...
    parser.add_argument('--output', type=str, default='data/causal_time_prior_shuffled_10k.pt')
    
    args = parser.parse_args()
    
    generate_dataset_shuffled(
        n_scms=args.n_scms,
        T=args.T,
